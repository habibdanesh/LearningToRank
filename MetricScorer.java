...
public abstract class MetricScorer {
	...
	public double score(List<RankList> rl)
	{
		...
		double score = 0.0;
		double sigmaWeights= 0.0;
		double scoresAndWeights = 0.0;
		double weight = 0.0;

		for(int i=0;i<rl.size();i++)
		{
			score += score(rl.get(i));
			weight = getImportenceWeight(rl.get(i));
			scoresAndWeights += score(rl.get(i)) * weight;
			sigmaWeights += weight;
		}
		
		return scoresAndWeights/sigmaWeights ;
    ...
    public double getImportenceWeight(RankList rl) 
    {
