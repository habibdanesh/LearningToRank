...
public class AdaRank extends Ranker {
	...
	private WeakRanker learnWeakRanker()
	{
		...
		for (int i : features) {
			...
			for (int j = 0; j < samples.size(); j++) {
				double t = scorer.score(wr.rank(samples.get(j))) * sweight[j] * impWeight[j];
	...
	private int learn(int startIteration, boolean withEnqueue)
	{
		...
		for(; t<=nIteration; t++)
		{
			...
			for (RankList sample : samples) {
				...
				trainedScore += tmp * impWeight[tempind++] ;
				//trainedScore += tmp;
			}
	...
	public void init()
	{
		...
		//begin added by Fatemeh
        //double[] impWeight = new double[samples.size()];
        impWeight = new double[samples.size()];
        double sumW = 0.0 ;
        for(int i=0;i<impWeight.length;i++){ 
            impWeight[i] = scorer.getImportenceWeight(samples.get(i));
            sumW += impWeight[i];
        }
        
        for(int i=0;i<impWeight.length;i++){
            impWeight[i] = (double)(impWeight[i]/sumW);
        }
